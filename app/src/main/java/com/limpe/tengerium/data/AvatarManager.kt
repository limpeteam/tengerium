...
class AvatarManager(
    ...
    // Кэш соответствия SafeSHA1 -> Путь к файлу
    private val safeSha1ToFileCache = ConcurrentHashMap<String, String>()
    ...
    private fun loadExistingAvatars() {
        avatarsDir.listFiles()?.forEach { file ->
            if (file.isFile && file.name.startsWith("av_") && file.name.endsWith(".dat")) {
                val safeSha1 = file.name.removePrefix("av_").removeSuffix(".dat")
                safeSha1ToFileCache[safeSha1] = file.absolutePath
            }
        }
    }
    ...
    private fun startWorker() {
        scope.launch {
            ...
            for (request in avatarRequestChannel) {
                ...
                val safeSha1 = toSafeSha1(sha1d)
                
                launch {
                    semaphore.withPermit {
                        try {
                            if (safeSha1ToFileCache.containsKey(safeSha1)) {
                                return@withPermit
                            }
                            ...
                            if (session != null) {
                                ...
                                inProgressSha1.remove(safeSha1)
                            }
                        } catch (e: Exception) {
    ...
    fun getAvatarBySha1(sha1: String): String? {
        return safeSha1ToFileCache[toSafeSha1(sha1)]
    }
    ...
    fun requestAvatar(account: String, msnObject: String) {
        ...
        if (safeSha1ToFileCache.containsKey(safeSha1)) return
        
        if (inProgressSha1.putIfAbsent(safeSha1, true) != null) return
    ...
    fun getAvatarFile(msnObject: String?): File? {
        ...
        val path = safeSha1ToFileCache[toSafeSha1(sha1d)] ?: return null
        val file = File(path)
        return if (file.exists() && file.length() > 0) file else null
    }

    private fun toSafeSha1(sha1d: String): String {
        return sha1d.replace("/", "_").replace("+", "-").replace("=", "").trim()
    }
